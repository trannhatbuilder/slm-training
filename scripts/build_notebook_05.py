#!/usr/bin/env python3
"""
Build notebooks/05_baseline.ipynb from scratch using nbformat.
...
OUTPUT_PATH = REPO_ROOT / "notebooks" / "05_baseline.ipynb"
...
CELL_0_MD = """# 05 — Base Model Baseline on benchmark_v1
...
try:
    ...
    REPO_ROOT = Path("/home/z/my-project/repos/slm-training")
...
CELL_2_MD = """## 1. Load benchmark

Load all 34 cases from `benchmark_v1.jsonl` and print a coverage summary.
"""

CELL_3_CODE = '''def load_benchmark(path: Path) -> list[dict]:
...
CELL_5_CODE = '''def load_base_model(model_id: str):
...
CELL_7_CODE = '''def build_prompt(case: dict) -> str:
...
CELL_12_MD = """## 6. Baseline run loop
...
CELL_13_CODE = '''def run_baseline(model, cases: list[dict]) -> list[dict]:
...
CELL_14_MD = """## 7. Aggregate and write baseline_result.json

Compute pass rate, error distribution, hard-negative pass rate, and write to `data/benchmark/baseline_result.json`.
"""

CELL_15_CODE = '''def aggregate(per_case: list[dict], cases: list[dict]) -> dict:
...
def main() -> int:
    ...
    assert len(nb2.cells) == len(nb.cells), "round-trip cell count mismatch"
    print("Validation OK — notebook reads back correctly.")
    return 0
